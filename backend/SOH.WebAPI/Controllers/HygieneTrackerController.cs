...
namespace SOH.WebAPI.Controllers
{
    ...
    {
        private readonly IHygieneTrackerService _hygiene;
        ...
        public override async Task<HygieneTrackerResponse?> GetById(int id)
        {
            await EnsureCallerMayAccessAsync(_hygiene, id);
            return await base.GetById(id);
        }

        // The owning patient comes from the JWT in HygieneTrackerService, so a
        // client-supplied PatientId cannot write into another patient's diary.
        ...
        [Authorize(Roles = RoleNames.Administrator + "," + RoleNames.Patient)]
        public override async Task<HygieneTrackerResponse?> Update(int id, [FromBody] HygieneTrackerUpsertRequest request)
        {
            await EnsureCallerMayAccessAsync(_hygiene, id);
